...
| **Look up domain reference / research** (code reading, business rules, terminology, verifying against domain facts) | Consult the internal **CaterSpot wiki first** if available: the `wiki-query` skill (compiled Obsidian vault), Confluence via the Atlassian MCP (`search`/`fetch`), or the `search-company-knowledge` skill. Detect availability with the Phase-1 detect-first probe. Only expand to external providers (the *Search the web* row below) for what the wiki does not cover. The wiki *sources* the rules a verifier encodes and an evaluator judges against — it does not replace the deterministic binary check. **Verify against current host docs.** | Point a Retrieval tool at the internal wiki corpus (Confluence export / vector index) and query it first; fall back to external `web_search` only for gaps. **Verify against current host docs.** | If the deployment wires an internal KB tool (wiki / Confluence connector), query it first; otherwise route through an MCP KB endpoint before any external search. **Verify against current host docs.** | Register the internal wiki as a read connector (`claw` tool or MCP) and query it before `WebSearch`. **Verify against current host docs.** |
...
| **Auto-update dashboard on tick** | Preferred: call `render_dashboard.py` inline at the end of the loop's write-state step (deterministic, no token cost). Decoupled: run `dashboard_watch.sh` as a background process (event-driven via `fswatch`, else an mtime poll). Host-native: a `Stop` hook can fire the render at the end of each turn — **verify the hook surface against current host docs.** | Codex has no persistent listener. Have the run write the state file, then invoke the renderer as a `bash` tool step in the same run, or re-render from the external poller that drives `run-until`. **Verify against current host docs.** | Add the render as a final workflow step after the state-write step, or attach it to Hermes's file-change / post-step trigger if the deployment exposes one. **Verify against current host docs.** | Invoke the renderer via `claw shell` at the end of the agent step, or register `dashboard_watch.sh` as a background service. **Verify against current host docs.** |
...
- **Internal knowledge before external providers.** For any research,
  code-reading, or domain-reference step, resolve the internal CaterSpot wiki
  first and treat external search as the fallback. This keeps loops grounded in
  CaterSpot's own business domain and avoids leaking internal context to
  third-party providers. Wire the loop's SKILL to the abstract *reference
  lookup* action above — never hard-bind it to one wiki tool name — so the
  precedence survives a host or tooling change. If no wiki is present, the
  action falls straight through to external search.
